...
subject to FullGrade_u{i in FullGrade}: WorkDivision[i] <= u_FullGrade[i];
# FullGrade percentage <= upper bound
subject to FullGrade_l{i in FullGrade}: WorkDivision[i] >= l_FullGrade[i];
# FullGrade percentage >= lower bound
subject to MyGrade_u{j in MyGrade}: sum {i in FullGrade} WorkDivision[i] * n_percent[i,j] / 100 <= u_MyGrade[j];
# MyGrades percentage <= upper bound
subject to MyGrade_l{j in MyGrade}: sum {i in FullGrade} WorkDivision[i] * n_percent[i,j] /100 >= l_MyGrade[j];
